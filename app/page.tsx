...
export default function Home() {
  ...
  return <main className="loginPage"><div className="overlay"/><form className="loginBox" onSubmit={submit}><Crown className="crown"/><h1>المتجر الأوربي في العراق</h1><small>EUROPEAN STORE IN IRAQ</small><h2>ادخل الآن<br/><span>وانضم إلى المزاد</span></h2><p>تسجيل سريع بالاسم ورقم الهاتف</p><label><User/><input required minLength={2} value={name} onChange={e=>setName(e.target.value)} placeholder="الاسم الكامل"/></label><label><Phone/><input required value={phone} onChange={e=>setPhone(e.target.value)} placeholder="07XXXXXXXX" inputMode="tel" pattern="07\d{8}" maxLength={10}/></label><small className="phoneHint">مثال: 07701234567</small>{error&&<div className="error">{error}</div>}<button className="goldBtn">دخول إلى المزاد</button></form></main>
}
