...
export default function ProductForm({ initial }: { initial?: ProductFormData | null }) {
  ...
  const [form, setForm] = useState({
    ...
    openingPrice: initial ? String(initial.openingPrice) : "",
    reservePrice: initial?.reservePrice ? String(initial.reservePrice) : "",
    bidIncrement: initial ? String(initial.bidIncrement) : "",
    startsAt: initial ? toLocalInput(initial.startsAt) : "",
    endsAt: initial ? toLocalInput(initial.endsAt) : "",
    blurImage: initial?.blurImage || false,
  });
  ...
  async function submit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      active: editing ? !!initial?.active : true,
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) {
        setError(typeof data.error === "string" ? data.error : "تعذر حفظ البيانات، يرجى مراجعة الحقول");
        return;
      }
      router.push("/admin");
      router.refresh();
  ...
  const title = editing ? "تعديل بيانات المنتج" : "إضافة منتج جديد للمزاد";
  const crumb = editing ? "تعديل المنتج" : "إضافة منتج جديد";
  const subtitle = editing
    ? "قم بتحديث تفاصيل القطعة، الصورة، الأسعار أو المواعيد ثم احفظ التغييرات"
    : "املأ تفاصيل القطعة ومواعيد مزايدتها والحد الأدنى للبيع لعرضها للجمهور";
    ...
    <main className="newProductAdmin">
      <header className="newProductHeader">
        <div className="newProductTitleArea">
          ...
          <h1>
            {editing ? <Save size={28} /> : <PackagePlus size={28} />}
            {title}
          </h1>
          <p>{subtitle}</p>
        </div>
                  ...
                  {uploading ? (
                    ...
                    <div className="uploadPreviewWrap">
                      ...
                      <div className="uploadSuccessTag">
                        ...
                        <span>{editing ? "الصورة الحالية" : "تم رفع الصورة بنجاح"}</span>
                      </div>
                      <button
                        type="button"
                      ...
                      >
                        <X size={14} />
                        {editing ? "رفع صورة أخرى" : "إزالة الصورة"}
                      </button>
                    </div>
                  ) : (
                    <div className="uploadPlaceholder">
